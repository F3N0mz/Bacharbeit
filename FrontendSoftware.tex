...
\begin{figure}
	...
	\caption[Model-View-ViewModel Structure]{Model-View-ViewModel Structure. This Figure explains what model consists of and how different parts of model interact with each other. Credit \cite{García2023}}
...
\begin{itemize}
	...
	\item \textbf{\ac{MVVM}} is an architectural design pattern that separates an application into three (Model, View, ViewModel) components to enhance modularity, testability, and maintainability. Advantages of this pattern is that it separates logic from presentation, allowing easier testing, debugging and troubleshooting\cite{García2023}.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=1\linewidth]{Figures/Screenshot_17}
	\caption[App Structure diagram]{Diagram of an Android App showing how its different components interact with each other and with backend. The structure is divided into 3 parts: View Layer, ViewModel Layer and Model Layer, corresponding to \ac{MVVM} paradigm.}
	\label{fig:screenshot17}
\end{figure}

\subsubsection{MainActivity.kt}
The Main entry file to the whole app is called \textbf{MainActivity.kt}. The fundamental part of the \ac{UI} is connected through this file as can be seen on Figure \ref{fig:screenshot17}. It is worth reviewing it here before delving deeper into the each view. The top level structure of the code can be seen in Listing \ref{lst:MainActivity}.
...
\textbf{DevConnViewModel.kt} is responsible for State management. Following the \ac{MVVM} pattern it is the ViewModel part of the app Besides the Factory, this is the sole ViewModel used for this app, as shown here \ref{fig:screenshot17}. It contains:
\begin{itemize}
...
\textbf{DevConnViewModelFactory.kt} file contains contains a single \textit{DevConnViewModelFactory} class. It is a custom factory responsible for creating instances of \texttt{DevConnViewModel} This is necessary because \texttt{DevConnViewModel} requires arguments in its constructor (\texttt{application} and \texttt{bluetoothLeManager}), which the default \texttt{ViewModel} provider mechanism cannot supply.
\newpage
...
\end{enumerate}
Many of these files (\textbf{BluetoothLeManager.kt}, \textbf{MainActivity.kt} and \textbf{GattAttributes.kt}) have already been previously covered and 3 files that will be covered next, to understand how the view is created.
\textbf{DevConnScreen.kt} file is responsible for layout and \ac{UI} elements. Its top level structure can be seen in Listing \ref{lst:DevConnVM} It contains:
\begin{itemize}
...
The Device Configuration screen has the following functionality, as also seen on Figure \ref{fig:devconfview}:
\begin{itemize}
...
\end{itemize}
All these interactions need an activle Bluetooth connection to the device. This view depends on these \ac{GATT} attributes to function:

\textbf{Read Characteristics}:
\begin{itemize}
	\item \textbf{GattAttributes.CHAR\_GET\_DEVICE\_TIME\_UUID} to display the "Current Device Time."
	\item \textbf{GattAttributes.CHAR\_GET\_DISPENSE\_SCHEDULE\_UUID} to display the "Current Dispense Schedule."
\end{itemize}

\textbf{Write Characteristics}
\begin{itemize}
	\item \textbf{GattAttributes.CHAR\_SET\_DEVICE\_TIME\_UUID} to send the new time string to the ESP32.
	\item \textbf{GattAttributes.CHAR\_SET\_DISPENSE\_SCHEDULE\_UUID} to send the new schedule string to the ESP32.
	\item \textbf{GattAttributes.CHAR\_TRIGGER\_MANUAL\_DISPENSE\_UUID} to command the ESP32 to dispense.
\end{itemize}
...
The \textbf{MonitoringStatusScreen} is designed to provide a read-only view of various operational data and logs from the connected pill dispenser. It allows the user to:
\begin{itemize}
	\item \textbf{View Current Device Time}	
	\item \textbf{View Current Dispense Schedule}
	\item \textbf{View Last Dispense Information} Displays details about the most recent dispense event 
	\item \textbf{View Time Until Next Dispense} Shows a countdown or status related to the next scheduled dispense (this is simulated/periodically updated by the ESP32 in \texttt{main.cpp} file on the Microcontroller side).
	\item \textbf{View Dispense Log} Shows a raw log of dispense events (currently not implemented, but was designed during project development phase)
	\item \textbf{Refresh Data} A button allows manually refreshing the "Last Dispense Info," 
\end{itemize}
This screen relies heavily on the ESP32 sending data (Timer/Counter information) via \ac{BLE} notifications. There are once again multiple files contributing to the functionality of this screen:
\begin{itemize}
	\item \textbf{DevMonitroingScreen.kt} defines the user interface for displaying monitoring information. Breaking the pattern, the main composable function is not the same as the name of the file, is called \texttt{MonitoringStatusScreen}. This is a minor oversight, which does not affect functionality of the code. It is the \textbf{View} layer of the \ac{MVVM} paradigm.
	\item \textbf{DevConnViewModel.kt} like in other screens, it manages UI-related data for the monitoring screen, interacts with \texttt{BluetoothLeManager} to get this data, and exposes it as observable flows. It is the \textbf{ViewModel} layer of the \ac{MVVM} paradigm.
	\item  \textbf{BluetoothLeManager.kt} here too, handles all \ac{BLE} communication. It is the \textbf{Model} layer of \ac{MVVM} paradigm.
\end{itemize}
For this screen to function, this View receives following \ac{GATT} characteristics:
\begin{itemize}
	\item \textbf{GattAttributes.CHAR\_GET\_DEVICE\_TIME\_UUID} to display "Device Time" 
	\item \textbf{GattAttributes.CHAR\_GET\_DISPENSE\_SCHEDULE\_UUID} to display "Dispense Schedule". 
	\item \textbf{GattAttributes.CHAR\_GET\_LAST\_DISPENSE\_INFO\_UUID} to display "Last Dispense Info". 
	\item \textbf{GattAttributes.CHAR\_GET\_TIME\_UNTIL\_NEXT\_DISPENSE\_UUID} to display "Time Until Next Dispense". 
	\item \textbf{GattAttributes.CHAR\_GET\_DISPENSE\_LOG\_UUID} to display "Dispense Log (Raw)". 
\end{itemize}
